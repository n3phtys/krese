...
class BusinessLogicTest {
    ...
    fun businesslogic_createwithverification() {
        ...
        assertEquals(response.successful, true)
        assertEquals(mailMock.sentMails.last().emailReceivers().get(0), creator.address)
        assertEquals(mailMock.sentMails.last().emailSubject(), "Reservation for PLACEHOLDER TITLE on 2018-04-01")
        assertEquals(mailMock.sentMails.get(mailMock.sentMails.size - 2).emailReceivers().get(0), moderator.address)
        assertEquals(mailMock.sentMails.get(mailMock.sentMails.size - 2).emailSubject(), "New Reservation for PLACEHOLDER TITLE on 2018-04-01")
    }
    ...
    fun businesslogic_acceptwithverification() {
        ...
        assertEquals(mailMock.sentMails.last().emailReceivers().get(0), creator.address)
        assertEquals(mailMock.sentMails.last().emailSubject(), "Your reservation for PLACEHOLDER TITLE on 2018-04-01 was accepted")
        assertEquals(mailMock.sentMails.get(mailMock.sentMails.size - 2).emailReceivers().get(0), moderator.address)
        assertEquals(mailMock.sentMails.get(mailMock.sentMails.size - 2).emailSubject(), "Reservation for PLACEHOLDER TITLE on 2018-04-01 was accepted")
    }
    ...
    fun businesslogic_declinewithverification() {
        ...
        assertEquals(mailMock.sentMails.size, 2)
        assertEquals(mailMock.sentMails.last().emailReceivers().get(0), creator.address)
        assertEquals(mailMock.sentMails.last().emailSubject(), "Reservation for PLACEHOLDER TITLE on 2018-04-01 was declined")
        assertEquals(mailMock.sentMails.get(mailMock.sentMails.size - 2).emailReceivers().get(0), moderator.address)
        assertEquals(mailMock.sentMails.get(mailMock.sentMails.size - 2).emailSubject(), "Reservation for PLACEHOLDER TITLE on 2018-04-01 was declined")
    }
    ...
    fun businesslogic_withdrawwithverification() {
        ...
        assertEquals(mailMock.sentMails.size, 2)
        assertEquals(mailMock.sentMails.last().emailReceivers().get(0), creator.address)
        assertEquals(mailMock.sentMails.last().emailSubject(), "Reservation for PLACEHOLDER TITLE on 2018-04-01 was withdrawn")
        assertEquals(mailMock.sentMails.get(mailMock.sentMails.size - 2).emailReceivers().get(0), moderator.address)
        assertEquals(mailMock.sentMails.get(mailMock.sentMails.size - 2).emailSubject(), "Reservation for PLACEHOLDER TITLE on 2018-04-01 was withdrawn")
    }
